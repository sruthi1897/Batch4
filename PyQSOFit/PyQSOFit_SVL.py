...
class QSOFit:
    ...
    def Fit(self, name=None, nsmooth=1, and_or_mask=True, reject_badpix=True, deredden=True, wave_range=None,
            ...
            poly=False, PL=True, BC=False, rej_abs=False, initial_guess=None, MC_conti=False,
    ...
    def _PlotFig(self, ra, dec, z, wave, flux, err, decomposition_host, linefit, tmp_all, gauss_result, f_conti_model,
            ...
            if self.MC:
                for p in range(int(len(gauss_result) / 12)):
                    if gauss_result[6 * (p-1) + 12] < 1200.:
                        ...
                        color = 'r'
                    plt.plot(wave, manygauss(np.log(wave), gauss_result[::2][p * 6:(p + 1) * 6][1:]) + f_conti_model,
                             color=color)
                ...
                for p in range(int(len(gauss_result) / 6)):
                    if gauss_result[6 * (p - 1) + 6] < 1200.:
                        ...
                        color = 'r'
                    plt.plot(wave, manygauss(np.log(wave), gauss_result[p * 6:(p + 1) * 6][1:]) + f_conti_model,
                             color=color)
                plt.plot(wave, manygauss(np.log(wave),
        ...
        if self.BC:
            plt.plot(wave, self.PL_poly_BC, 'y', lw=2, label='BC')
        plt.plot(wave, self.PL_poly, color='orange', lw=2, label='conti')
